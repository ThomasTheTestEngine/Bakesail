...
class Bakesail:
    ...
    def __init__(self, config):
        ...
        # ── Bakesail FS server (advanced file browser, port 7127) ─────────────
        try:
            _start_fs_server()
        except Exception as e:
...
# =============================================================================
# Bakesail FS Server — lightweight file-browser API on port 7127
#
# Proxied by nginx at /bakesail/ so the frontend never makes cross-origin
...
#   POST /delete?path=…      → delete file or directory (body: {"recursive":false})
...
FS_PORT    = 7127
_fs_thread = None   # module-level guard — only one server per process


class _FSHandler(http.server.BaseHTTPRequestHandler):
    """Minimal HTTP handler for filesystem operations."""
    ...
    def do_GET(self):
        p = self.path.split('?', 1)
        ep   = p[0].rstrip('/')
        qs   = urllib.parse.parse_qs(p[1]) if len(p) > 1 else {}
        get  = lambda k, default='': qs.get(k, [default])[0]

        if   ep == '/info':     self._info()
        elif ep == '/list':     self._list(get('path', '/'))
    ...
    def do_POST(self):
        ...
        if   ep == '/upload': self._upload(get('dir', '/'))
    ...
    def _download(self, path):
        ...
        try:
            ...
            logging.warning("Bakesail FS download error: %s", e)

    def _upload(self, dest_dir):
        dest_dir = os.path.realpath(os.path.expanduser(dest_dir))
        if not os.path.isdir(dest_dir):
            return self._err(400, 'Destination is not a directory: ' + dest_dir)
        ct = self.headers.get('Content-Type', '')
        cl = int(self.headers.get('Content-Length', 0) or 0)
        body = self.rfile.read(cl)
        # Parse multipart using email module (cgi.FieldStorage deprecated in 3.11+)
        msg_text = ('Content-Type: ' + ct + '\r\n\r\n').encode() + body
        try:
    ...
    def _delete(self, path):
        ...
        if not os.path.exists(path):
            return self._err(404, 'Path not found: ' + path)
        try:
...
def _start_fs_server():
    """Start the FS HTTP server in a daemon thread (idempotent)."""
    global _fs_thread
